<script setup lang="ts">
  ...
  async function favorite(favorite: Favorite) {
    ...
    if (isFavoriteh) {
      ...
      await FavoriteRepository.createFavorite(favorite);
    }
    favorites.value = await FavoriteRepository.getRawFavorites();
    ...
    <template #content>
      <div class="relative overflow-hidden" style="min-height: 250px">
        ...
        >
          ...
          <div
            ref="divSearch"
            class="w-full h-48 flex flex-col overflow-y-scroll overflow-x-hidden"
          >
        ...
        >
          <div class="w-full flex">
            ...
            />
            <div v-if="favoriteView" class="w-[calc(100%-2rem)] p-2 flex">
              <NuxtImg
                :src="favoriteView.cover"
                class="w-40 h-56 object-contain rounded-xl select-none"
                draggable="false"
              />
              <div class="w-full flex flex-col justify-center p-2 gap-2">
                <UBadge class="text-center" color="neutral" variant="outline">
                  {{ favoriteView.name }}
                </UBadge>

                <UBadge
                  v-if="
                    favoriteView.name !== favoriteView.extra_name &&
                ...
                <UBadge class="text-center" color="neutral" variant="soft">
